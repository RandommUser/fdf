...
{
	map->cam.plan = vec4_ini((long double[4]){0, 100, FOV_DEF, -1});
	map->h_mod = H_MOD;
	map->zoom = ZOOM_DEF;
	map->limit = 1;
	if (map != mlx->map[0])
...
void	settings_reset(t_map *map, t_mlx *mlx)
{
	...
	mlx->smap = mlx->map[0];
	map->thick = 0;
	map->rot = vec4_ini((long double[4]){ROTA_X, ROTA_Y, ROTA_Z, ROTA_W});
	map->mode = MODE_DEF;
	map->h_mod = H_MOD;
	map->pos = vec4_ini((long double[4]){mlx->width, mlx->height, 0, 0});
	map->zoom = ZOOM_DEF;
	zoom_check(map);
	map->cam.loc = vec4_ini((long double[4]){(int)(map->pos.vec[0] / 2),
		(int)(map->pos.vec[1] / 2), 0, 1});
	map->cam.plan = vec4_ini((long double[4]){0, 100, FOV_DEF, -1});
	map->cam.rot = vec4_ini((long double[4]){90, 90, 0, 1});
	map->origin = mat4_trans((long double[3]){mlx->width / 2, mlx->height / 2, 1});
	map->limit = 1;
	map->color = 0;
